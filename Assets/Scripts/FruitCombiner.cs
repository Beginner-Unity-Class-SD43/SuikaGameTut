...
public class FruitCombiner : MonoBehaviour
{
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        {
            FruitInfo otherInfo = collision.gameObject.GetComponent<FruitInfo>();
            if(otherInfo != null)
            {
                if(otherInfo.FruitIndex == info.FruitIndex)
                {
                    ...
                    if(thisID > otherID)
                    {
                        GameManager.instance.IncreaseScore(info.PointsWhenCombined); // This is only for increasing score
